...
describe('when TodoList is rendered', () => {
  ...
  test('typing in the new form textboxes updates form values', () => {
    act(() => userEvent.type(screen.getByPlaceholderText('New Title'), 'TITLE'));
    act(() => userEvent.type(screen.getByPlaceholderText('New Description'), 'DESCRIPTION'));
  ...
  test('adding multiple items and clicking Edit sets edit form with that item\'s values', () => {
    act(() => userEvent.type(screen.getByPlaceholderText('New Title'), 'TITLE 1'));
    act(() => userEvent.type(screen.getByPlaceholderText('New Description'), 'DESCRIPTION 1'));
    act(() => userEvent.click(screen.getByText('Add New')));
    act(() => userEvent.click(screen.getByText('Add New')));
    act(() => userEvent.type(screen.getByPlaceholderText('Edit Title'), 'TITLE 2'));
    act(() => userEvent.type(screen.getByPlaceholderText('Edit Description'), 'DESCRIPTION 2'));
    act(() => userEvent.click(screen.getAllByText('Edit')[0]));
  ...
  test('deleting active item removes item from list and resets form', () => {
    ...
    act(() => userEvent.type(screen.getByPlaceholderText('Edit Title'), 'TITLE 2'));
    act(() => userEvent.type(screen.getByPlaceholderText('Edit Description'), 'DESCRIPTION 2'));
    act(() => userEvent.click(screen.getAllByText('Delete')[1]));
